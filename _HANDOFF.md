...
## 📌 고정 요구사항 (변경 금지 · 사장님 지시 2026-07-14)
아래 두 화면은 **현재의 형태·구성을 그대로 유지**한다. 389(외부 MES)나 리스타일을 하더라도 이 두 화면은 **현재 것을 기준**으로 하고 컬럼/구성을 바꾸지 않는다.
1. **수주(PO)등록·변경관리 = `수주관리.html`** — 컬럼: 수주번호·수주일자·거래처·설비명·품번(BOM)·품명·수량·단가·금액·납기일·진행상태 + 결재(작성→검토→검토→승인) + 진행/완료 집계. 저장: app_state.
2. **생산계획현황 = `생산계획현황.html`** — 자재소요 / 금형·C/T·UPH / 월계획(주간·야간·합계EA·순위) 다중표. 저장: app_state.
